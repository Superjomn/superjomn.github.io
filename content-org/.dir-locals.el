...
\usepackage{url}
\begin{document}
\title{DRWS: A Model for Learning Distributed Representation for Word and Sentence}
\maketitle
...
Feature vectors for words and sentences are learned based on their probability of co-occurring of the words and  sentence using a neural network.
...
These models model the distribution for the next word as a smooth function of learned multi-dimensional real-valued representations of the context words and the target word.
...
Huang\cite{huang2012improving} introduces a new neural-network-based language model (WRGC) that learns word's representation  with both local and global contexts via a joint training objective.  Directly inspired by the concept of global context, 
...
To represent sentences with vectors, 
    we assume that there are two important points: the sentence's representation should 1) contain all the meanings of words in it. 2) record the words' order.
Therefore, we proposed a model learned word vector representation first, 
    then we embed sentence's vector to the local context and global context, 
    frozen model parameters and continue the training.
    In this way, the semantics captured in word vectors will be incorporated into sentence's representation.

We evaluate our new model on the standard WordSim-353 dataset which includes human similarity judgments on pairs of words, showing that combining both local and sentence as global context can learn better representation of words.
...
However the method doesn't work for sentence-level representations for the uniqueness of sentence.
...
Huang\cite{huang2012improving} proposed the model(WRGC) that learns word's representation with both local context and global contexts, 
...
Given a sequence of words $s$ (local context) and document $d$ (global context) in which the sequence occurs, 
    WRGC's objective is to discriminate the correct last word in $s$ from other random selected words $s^*$.
...
\begin{figure}[htbp]
  \centering
   ...
   \caption{An overview of the model.
      ...
      The model uses fixed-width sliding window as local context and sentence as global context to compute a score that should be large for the actual next word(bank in the example),
...
\begin{equation}
\label{eq:max-score}
C_{v,s} = \sum_{w\in V} \max(0, 1-g(s,d) + g(s^*, d)).
\end{equation}
WRGC treat document as global context, and the representation of document is a weighted average of the vectors of all word in the document:
\begin{equation}
    d = \sum_{i=1}^{k} w(t_i) x_{t_i}
\end{equation}
where $t_i$ is the $i$-th word in document, 
...
\section{DRWS: new model with sentence as global context}
While WRGC treat document as the global context,
    we think sentence is a more appropriate choice because it has a closer relationship with the local context,
    and can be viewed as a superset of the local contexts in it.

In this section, we describe the details of DRWS based on WRGC.
DRWS learns word vector representation first,
then with a empty sentence embedding matrix added, it continues to train and fill the matrix with semantics learned in the word vector representations.




\subsection{Pretrain Word Vector}
We replace document $d$ with sentence $h$ in \eqref{eq:max-score}\eqref{eq:global-context},
...
\end{equation}
and $h$ indicates the vector representation of the sentence where the local context in,
...
We use Noise Contrastive Estimation (NCE)\cite{mnih2012fast}\cite{mikolov2013distributed} to sample noise word $w^*$ and generate a wrong word sequence $s^*$ in \eqref{eq:new-max-score}.  
...
all the model parameters are denoted as $\theta$ including word embedding matrix $L$ and neural network parameters $W_1, W_2, W^g_1, w^g_2, b_1, b_2, b^g_1, b^g_2$ are updated during training:
\begin{equation}
...
After words' vectors are learned,
    we get word vector representations and a neural network that captured semantic and syntax of the corpus.
...
\end{enumerate}
and finally get a cleaned corpus.
...
\begin{table}[h]
\caption{DRWS-0.5 is our model with $\lambda=0.5$ in \eqref{final-score} which is the final score formula in WRGC. Ourmodel-0.3 is our model with $\lambda=0.3$.}
\centering
...
Note that trained on mixed-corpus, DRWS-0.5 has the same final score formula is the same to WRGC, but get higher score(65.2), showing the effectness of sentence as global context. 
...
Notice that, our model only cares about local context when $\lambda = 0$, and it training with only global context when $\lambda=1.0$. 
It shows that trains only with local context has a better performance than with only global context, 
...
The cleaned corpus helps our model to get the best score $71.6$, which is a $4.8\%$ improvement compared to uncleaned corpus.
...
\begin{table}[h]
\caption{DRWS-0.5 is our model with $\lambda=0.5$ in \eqref{final-score} which is the final score formula in WRGC. DRWS-0.6 is our model with $\lambda=0.6$.}
\label{table:word-vector}
...
DRWS with $\lambda=0.6$ get the best result, and have an improvement of $3\%$ on ROUGE-1.
